...
This template tag, when implemented, converts a duration (in seconds) to a more
meaningful format. It has a short and long setting, which is easy to manipulate
for your needs.

http://djangosnippets.org/snippets/1398/
"""
...
def sectodur(value, arg=''):
    ...
    if secs > 0:
        ...
        # (minus number of days and hours)
        ...
        if days > 0:
            ...
            durationString += ' ' + str(days) + dayUnitName + \
                (days > 1 and 's' or '')
        ...
        if hours > 0:
            ...
            # If there are unit after this
                ...
                hourSplitter = \
                    (len(durationString) > 0 and nextDurSplitter or '')

        # If number of hours is greater than 0
        if hours > 0:

            # Add multiple days to duration string
            durationString += hourSplitter + ' ' + str(hours) + \
                hourUnitName + (hours > 1 and 's' or '')
        ...
        if minutes > 0:
            ...
            # Add multiple days to duration string
            durationString += minSplitter + ' ' + str(minutes) + \
                minUnitName + (minutes > 1 and 's' or '')
        ...
        if seconds > 0:
            ...
            # Add multiple days to duration string
            durationString += secSplitter + ' ' + str(seconds) + \
                secUnitName + (seconds > 1 and 's' or '')
